...
from data.get_data import GetTaskDB
from data.database import AddTaskDB
...
from keyboards.admin.view_data import button_next
...
@reg_router.message(F.text == 'test')
async def test(message:Message):

    await message.answer('ras ras',reply_markup=button_next())
...
async def text_opros(message:Message):
    ...
    add_task_db.add_answer(answer=option[0],task_id=get_task_id.get_id_two(content=content))
